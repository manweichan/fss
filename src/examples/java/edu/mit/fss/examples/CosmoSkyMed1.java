...
public class CosmoSkyMed1 extends DefaultFederate {
	 ...
	 * COSMO-SkyMED-1 federate objects and launches the associated graphical 
	 * user interface.
	 *
	...
	public static void main(String[] args) throws RTIexception, URISyntaxException {
		...
		final CosmoSkyMed1 federate = new CosmoSkyMed1();
		...
		try {
			...
			while(br.ready()) {
				if(br.readLine().matches(".*COSMO-SKYMED 1.*")) {
					logger.debug("Found COSMO-SKYMED 1 data.");

					logger.trace("Adding COSMO-SKYMED 1 space system.");
					system = new SpaceSystem("Cosmo SkyMED 1", 
							new TLE(br.readLine(), br.readLine()), 5123e3);
					federate.addObject(system);

					try {
						logger.trace("Setting inital time.");
				        federate.setInitialTime(system.getInitialState().getDate().toDate(
		...
		federate.getConnection().setFederateName("COSMO-SkyMED-1");
	...
	/**
	 * Instantiates a new COSMO-SkyMED-1 federate using a 
